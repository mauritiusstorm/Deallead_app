      ...
      <div className="mt-4 flex flex-col gap-3">
        {items.map((item) => (
          <div key={item.variantId} className="flex items-center gap-3">
            <div className="size-16 shrink-0 overflow-hidden rounded-md bg-white/10">
              {item.imageUrl && <img src={item.imageUrl} alt="" className="size-full object-cover" />}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm">{item.title}</p>
              <p className="text-xs text-white/50">{item.variantLabel}</p>
              <div className="mt-1 flex items-center gap-2">
                <button
                  onClick={() => setQuantity(item.variantId, item.quantity - 1)}
                  className="size-6 rounded-full border border-white/25 text-xs"
                >
                  −
                </button>
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              <button onClick={() => removeItem(item.variantId)} className="text-xs text-white/40 hover:text-white/70">
                Retirer
              </button>
            </div>
          </div>
